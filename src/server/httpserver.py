...
class S(BaseHTTPRequestHandler):
    ...
    def __init__(self, *args):
        ...
        self.line_trainers = {}
        self.model1_heros = ['27']
        self.model2_heros = ['28']
        self.real_heros = None
        self.build_models(model1_path=None,
                          model2_path=None,
                          initial_p=0.5,
                          final_p=0)
    ...
    def do_GET(self):
        ...
        if raw_state_info.battleid not in self.line_trainers:
            self.line_trainer[raw_state_info.battleid] = LineTrainer(self.save_dir, self.model1_heros, self.model1,
                                                                     self.model1_save_header,
                                                                     self.model2_heros, self.model2,
                                                                     self.model2_save_header
                                                                     )
        # 交给对线训练器来进行训练
        rsp_str = self.line_trainer[raw_state_info.battleid].train_line_model(get_data)
        print(rsp_str)
        print('\n')
    ...
    # 两个模型用来相互对战，分别训练
    def build_models(self, model1_path=None, model2_path=None, initial_p=1.0, final_p=0.02):
        # 创建存储文件路径
        date_str = str(datetime.now()).replace(' ', '').replace(':', '')
        self.save_dir = '/Users/sky4star/Github/zy2go/battle_logs/model_' + date_str
        os.makedirs(self.save_dir)

        # 创建模型，决定有几个模型，以及是否有真人玩家
        # 模型需要指定学习的英雄，这里我们学习用该模型计算的英雄加上真人（如果存在），注意克隆数组
        self.model1 = LineModel_DQN(279, 48, self.model1_heros, scope="linemodel1", initial_p=initial_p, final_p=final_p)
        if model1_path is not None:
            self.model1.load(model1_path)
        self.model1_save_header = self.save_dir + '/line_model_1_v'

        if self.model2_heros is not None:
            self.model2 = LineModel_DQN(279, 48, self.model2_heros, scope="linemodel2", initial_p=initial_p, final_p=final_p)
            if model2_path is not None:
                self.model2.load(model2_path)
            self.model2_save_header = self.save_dir + '/line_model_2_v'
        else:
            self.model2 = None
