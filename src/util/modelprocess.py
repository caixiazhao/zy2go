...
def start_model_process(battle_id_num, init_signal, train_queue, action_queues, result_queues, save_batch, save_dir):
    model_1, model1_save_header, model_2, model2_save_header = HttpUtil.build_models_ppo(
            ...
            # model1_path='/Users/sky4star/Github/zy2go/data/20171211/model_2017-12-07163246.528784/line_model_1_v400/model',
            # model2_path='/Users/sky4star/Github/zy2go/data/20171211/model_2017-12-07163246.528784/line_model_2_v400/model',
        ...
        try:
            ...
            if trained:
                ...
                for battle_id in range(0, battle_id_num):
                    # 给每个客户端添加一个训练结束的通知
                    if battle_id not in trained_signals:
                        trained_signals.append(battle_id)
                    continue
            ...
            act_model_names = []
            state_inputs = []
            for index, request_queue in enumerate(action_queues):
                while not request_queue.empty():
                    # 计算模型行为
                    (battle_id, act_model_name, state_input) = request_queue.get()
                    battle_ids.append(index)
            ...
            for battle_id, act_model_name, actions, vpred in zip(battle_ids, act_model_names, actions_list, vpreds):
                if battle_id in trained_signals and act_model_name == ModelProcess.NAME_MODEL_1:
                    # 提供训练完毕信号
                    print('modelprocess', battle_id, act_model_name, '提供训练完毕信号')
                    trained_signals.remove(battle_id)
                    restartCmd = CmdAction(ModelProcess.NAME_MODEL_1, CmdActionEnum.RESTART, 0, None, None, None,
                                           None, None, None)
                    result_queues[battle_id].put((battle_id, act_model_name, restartCmd, None, None))
                else:
                    result_queues[battle_id].put((battle_id, act_model_name, actions, explor_value, vpred))
        except queue.Empty:
            continue
...
class ModelProcess:
    ...
    def start(self):
        p = Process(target=start_model_process, args=(self.battle_id_num, self.init_signal, self.train_queue,
                                                      self.action_queues, self.result_queues,
