...
class LinePPOModel(object):
    ...
    def _init(self, ob_space, ac_space):
        ...
        out = ob
        out = tf.layers.fully_connected(out, num_outputs=256, activation_fn=tf.nn.relu, weights_initializer=U.normc_initializer(1.0))
        out = tf.layers.fully_connected(out, num_outputs=128, activation_fn=tf.nn.relu, weights_initializer=U.normc_initializer(1.0))
        out = tf.layers.fully_connected(out, num_outputs=128, activation_fn=tf.nn.relu, weights_initializer=U.normc_initializer(1.0))
